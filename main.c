#include "Chapter_03/chapter_03.h"
#include "Chapter_04/chapter_04.h"

int main() {
    main_prog_3_1_1();
    main_prog_3_1_2();
    main_prog_3_1_3();
    main_prog_3_1_4();
    main_prog_3_1_5();

    main_prog_3_2_1();
    main_prog_3_2_2();

    //main_prog_3_3_1();      // input
    //main_prog_3_3_2();      // input
    //main_prog_3_3_3();      // input

    //main_prog_3_4_1();      // input
    //main_prog_3_4_2();      // input

    //main_prog_3_5_1();      // input
    //main_prog_3_5_2();      // input
    //main_prog_3_5_3();      // input
    //main_prog_3_5_4();      // input
    //main_prog_3_5_5();      // input
    //main_prog_3_5_6();      // input
    //main_prog_3_5_7();      // input
    //main_prog_3_5_8();      // input

    //main_prog_4_3_1();      // input
    //main_prog_4_3_2();      // input

    //main_prog_4_4_1();      // input
    //main_prog_4_4_2();      // input
    //main_prog_4_4_3();      // input

    //main_prog_4_8_1();      // input
    //main_prog_4_8_2();      // input
    //main_prog_4_8_3();      // input
    //main_prog_4_8_4();      // input

    //main_prog_4_10_1();     // input
    //main_prog_4_10_2();     // input
    //main_prog_4_10_3();     // input
    //main_prog_4_10_4();     // input
    main_prog_4_10_5();     // input

    return 0;
}
